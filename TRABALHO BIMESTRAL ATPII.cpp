...
int BuscaBinariaLocacaoCPF(TpLocacao Loc[TF], int TL, char Aux[15])
{
	int I=0,M,F=TL-1;
	M=F/2;
...
//EX1
void ClieteComTresOuMaisLocacao(TpLocacao Loc[TF],int TL){
...
void ExclusaoLocacao(TpLocacao Loc[TF], int TL){
	...
	printf("/nDigite um CPF para excluir a Locacao: ");
	gets(Aux);
	while(strcmp(Aux,"\0")!=0){
		...
		if(pos == -1){
			...
			printf("\nDigite o Codigo para terminar a exclusao: ");
