    ...
    while (TL < TF && strlen(AuxCPF) > 0)
    {
        ...
        else
        {
            ...
            do {
                ...
                if (novoCodigo == Loc[pos].Codigo && novoCodigo != 0) {
                ...
                else if (novoCodigo != 0 && buscaCarro(Car, TP, novoCodigo) == -1) {
                    gotoxy(4,18);
                    ...
                    novoCodigo = Loc[pos].Codigo; 
                     ...
                     (novoCodigo == Loc[pos].Codigo || buscaCarro(Car, TP, novoCodigo) == -1));
            ...
            int posCar = buscaCarro(Car, TP, Loc[pos].Codigo);
            if (posCar != -1) {
...
void ExclusaoLocacao(TpLocacao Loc[TF], int &TL){
    ...
    while(strcmp(Aux, "\0") != 0){
        ...
        if(pos == -1){
            ...
            printf("CPF ENCONTRADO");
            gotoxy(4,10);
            printf("%s %d %d %.2f", Loc[pos].CPF, Loc[pos].Codigo, Loc[pos].Dias, Loc[pos].ValorLocacao);
			gotoxy(4,11);
            printf("Digite o Codigo para confirmar exclusao: ");
            scanf("%d", &AuxCod);
            ...
            if(Loc[pos].Codigo == AuxCod){
				...
				printf("### EXCLUI LOCACAO ###");
            	gotoxy(27,11);
                printf("CODIGO NAO CONFERE, EXCLUSAO CANCELADA");
                Sleep(3000);
            }
        }
